...
export async function PATCH(request: Request, { params }: Ctx) {
  ...
  if (statut !== "ACCEPTE" && statut !== "REFUSE") return err("Statut invalide")
  ...
  if (devis.projet.client.userId !== userId!) return err("Non autorisé", 403)
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    if (statut === "ACCEPTE") {
      ...
      await tx.chantier.create({
        data: { projetId: devis.projetId, devisId: id, artisanId: devis.artisanId },
      })
    }
