    ...
    <div className="pt-24 pb-16">
      ...
      <div className="bg-[#0F2C5E] py-16 mb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center text-white">
          <p className="text-[#6B7280] font-semibold text-sm uppercase tracking-widest mb-3">Ce que nous proposons</p>
          ...
          <div key={s.slug} className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${i % 2 === 1 ? "lg:flex-row-reverse" : ""}`}>
            ...
            <div className={i % 2 === 1 ? "lg:order-1" : ""}>
              ...
              <ul className="space-y-2 mb-8">
                {s.items.map((item) => (
                  <li key={item} className="flex items-center gap-2 text-sm text-gray-700">
                    <span className="w-5 h-5 bg-[#6B7280] rounded-full flex items-center justify-center text-white text-xs shrink-0">✓</span>
                    {item}
                  </li>
                ))}
              </ul>
              <div className="flex gap-3">
                ...
                <Link href="/devis" className="bg-[#6B7280] text-white font-semibold px-5 py-2.5 rounded-xl hover:bg-gray-600 transition-colors text-sm">
                  Devis gratuit
                </Link>
              </div>
            </div>
